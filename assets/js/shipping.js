...
const ShippingModule = {
    ...
    async render() {
        ...
        const shippingOrders = this.getShippingOrders().sort((a, b) => {
        ...
        if (this._tab === 'carriers') {
            ...
            html += this._renderShipments(shippingOrders, formatCurrency);
        }
        ...
        <div class="bg-white rounded-2xl p-6 border border-gray-100" style="box-shadow:0 1px 4px rgba(0,0,0,0.03);">
            <div class="flex items-center justify-between mb-5">
                <h3 class="font-bold text-gray-900 flex items-center gap-2 text-sm">
                    ...
                    الشحنات (${orders.length})
                </h3>
            </div>`;

        if (orders.length === 0) {
            html += `<p class="text-center text-gray-300 py-8"><i class="fas fa-box-open text-4xl mb-2 block"></i><span class="text-sm">لا توجد شحنات</span></p>`;
        } else {
            ...
            orders.forEach(o => {
                ...
                html += `<tr class="cursor-pointer hover:bg-gray-50" onclick="OrdersModule.openOrderDetail(${o.id})">
                    <td class="font-bold text-accent">#${o.id.toString().slice(-5)}</td>
                    <td>${o.clientName || '-'}</td>
